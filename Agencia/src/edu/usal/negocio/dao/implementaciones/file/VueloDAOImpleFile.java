...
//		, Date fechSalida, Date fechLlegada, String horasVuelo, String idLAerea
		...
		for (String s : IOfile.cargarLista("Vuelo")) {
			...
			fechSalida = IOGeneral.convertirADate(splitted[6]);
			fechLlegada = IOGeneral.convertirADate(splitted[7]);
...
//					 Date fechSalida, Date fechLlegada, String horasVuelo)
			lVueloDAO.add(new Vuelo(idVuelo, numeroVuelo, asientosTotales, asientosDisponibles,
					 fechSalida, fechLlegada,horasVuelo));
	...
	public void guardarVuelos(List<Vuelo> lVuelos) {
		...
		for (Vuelo v : lVuelos) {
			...
			lString.add(v.getIdVuelo() + "," + v.getNumeroVuelo() 
				 + "," + v.getAsientosTotales() + "," + v.getAsientosDisponibles()
				 + "," + v.getAeropuertoSalida().getIdAeropuerto()
				 + "," + v.getAeropuertoLlegada().getIdAeropuerto()
				 + "," + v.getFechSalida().getDate() + "/" + (v.getFechSalida().getMonth()+1) 
				 + "/" + v.getFechSalida().getYear()
				 + "," + v.getFechLlegada().getDate() + "/" + (v.getFechLlegada().getMonth()+1)
				 + "/" + v.getFechLlegada().getYear()
				 + "," + v.getHorasVuelo() + "," + v.getLineaAereaVuelo().getIdLAerea());
		}
