...
//			, String nroPF, String email, int idDireccion
			...
			cuil = splitted[5];
			fechNac = IOGeneral.convertirADate(splitted[6]);
			idTelefono = Integer.parseInt(splitted[7]);
			nroPF = splitted[8];
			email = splitted[9];
			idDireccion = Integer.parseInt(splitted[10]);
			
//			Cliente(int idCliente, String nombre, String apellido, String dni, String cuil, 
//			Date fechNac, String email)	
	...
	public void guardarClientes(List<Cliente> lClientes){
		...
		lString.add("#int idCliente, String nombre, String apellido"
		...
		for (Cliente c : lClientes) {
			...
			lString.add(c.getIdCliente() + "," + c.getNombre() + "," + c.getApellido()
			 + "," + c.getDni() + "," + c.getPasaporte().getnroPasaporte()
			 + "," + c.getCuil() + "," 
			 + c.getFechNac().getDate() + "/" + (c.getFechNac().getMonth()+1) 
				+ "/" + c.getFechNac().getYear()
			 + "," + c.getTelefono().getIdTelefono() + "," + c.getPasaporte().getnroPasaporte()
